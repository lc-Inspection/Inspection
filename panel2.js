    ...
    kayitlar.forEach(function(k, i) {
      ...
      html += '<tr style="background:' + bg + ';">'
        ...
        + '<td style="font-family:monospace;font-weight:700;text-align:center;">'
          + (oran !== null ? '<span style="color:' + _aoPerfClass(oran) + ';">' + oran + '%</span><div style="font-size:9px;font-weight:400;color:' + _aoPerfClass(oran) + ';">' + (k.is2KalitePerf80 ? '🔶 2. Kalite' : (oran >= 100 ? '✓ ' + (translations[currentLang]||translations.tr).on_target : '↓ ' + (translations[currentLang]||translations.tr).below_target)) + '</div>' : '<span style="color:#5A7FA8;">—</span>')
          + '</td>'
        + '</tr>';
    });
  });
